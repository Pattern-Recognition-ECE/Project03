function [centers] = KMeans2(Vector,KM,Threshold)
%#
...
if ( NumOfVect <= KM )
	disp( 'The size of the alphabet is greater than the number of the Vectors' ) ;
	return ;
end
...
while( (prError-Error)/Error > Threshold ) 
    ...
    for i = 1:KM
        for j = 1:NumOfVect
            dist = (Alphabet(:,i) - Vector(:,j))' * (Alphabet(:,i) - Vector(:,j));
            curDist = (Alphabet(:,i) - centers(:,i))' * (Alphabet(:,i) - centers(:,i));
            if dist < curDist
                centers(:,i) = Vector(:,j);
            end
        end
